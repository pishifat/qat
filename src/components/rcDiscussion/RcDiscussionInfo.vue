            ...
            <div class="modal-body" style="overflow: hidden">
                ...
                <div v-if="rcDiscussion.isActive && (userModes.indexOf(rcDiscussion.mode) >= 0 || rcDiscussion.mode == 'all' || isLeader)">
                    ...
                    <div class="mb-2">
                        ...
                        <p class="small ml-2">Only NAT leaders can see your vote. If you have any feedback to improve the proposal, post on the thread.</p>
                    </div>
                    <div :class="info.length ? 'errors' : 'confirm'" class="text-shadow ml-2" style="min-height: 24px;">{{info}} {{confirm}}</div>
                </div>
                <p v-else-if="rcDiscussion.isActive" class="small">Because you're not proficient in this proposal's game mode, you're not able to vote :(</p>
            </div>
            <div class="modal-footer">
                <button v-if="rcDiscussion.isActive && (userModes.indexOf(rcDiscussion.mode) >= 0 || rcDiscussion.mode == 'all' || isLeader)" class="btn btn-sm btn-nat" @click="submitMediation($event)">Submit Vote</button>
                <p v-else class="text-shadow min-spacing">{{ rcDiscussion.createdAt.slice(0, 10) }}</p>
            </div>
        </div>
    </div>
</div>
